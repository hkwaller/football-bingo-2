...
function kindTabClass(kind: string | null) {
  const base =
    'inline-flex items-center rounded-full px-2.5 py-1 text-[9.5px] font-extrabold uppercase leading-none tracking-[0.12em]'
  if (kind === 'nationality') return `${base} bg-sky text-pitch-deep`
  if (kind === 'club') return `${base} bg-green-go text-white`
  return `${base} bg-yellow text-pitch-deep`
}
                ...
                >
                  {logo ? (
                    <Image
                      ...
                      width={38}
                      height={38}
                      className="h-8 w-8 object-contain sm:h-[38px] sm:w-[38px]"
                      unoptimized
                    />
                  ) : (
                    <span className="text-[28px] leading-none sm:text-[30px]" aria-hidden>
                      🏆
                    </span>
                  )}
                  <span className={kindTabClass(kind)}>{KIND_SHORT[kind ?? ''] ?? 'Square'}</span>
                  <span className="line-clamp-3 px-0.5 text-[11px] font-extrabold uppercase leading-tight tracking-[0.01em] text-card-ink sm:text-[12.5px]">
