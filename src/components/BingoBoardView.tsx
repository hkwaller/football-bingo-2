...
import type { CategoryKind } from '@/lib/canonical'
import type { BingoCellView } from '@/lib/bingoCellView'
import type { CellPick } from '@/lib/cellPick'

type BingoBoardViewProps = {
  cells: BingoCellView[]
  size: number
  /** Re-keys the squares when the board changes (the seed, on a live board). */
  boardKey: string
  /** Squares on a completed line. */
...
/** Draws a board from resolved squares. BingoBoard wraps this for live games. */
    ...
    <div className="relative mx-auto w-full max-w-[760px] overflow-hidden rounded-xl border-[2.5px] border-ink bg-pitch-deep/90 p-[7px] shadow-[0_6px_0_#0a2417] sm:rounded-2xl sm:p-[18px] sm:shadow-[0_10px_0_#0a2417]">
      ...
      >
        {cells.map((cell, index) => {
          ...
          const logo = cell.kind === 'category' ? cell.logo : null
          const catKind = cell.kind === 'category' ? cell.catKind : null
          const display = cell.kind === 'category' ? cell.display : ''
