...
async function apiFetch<T>(endpoint: string): Promise<T | null | false> {
  try {
    ...
    if (res.status === 405 || res.status === 403 || res.status === 429) {
      console.warn(
        `  ⚠ Rate limited (${res.status}). Waiting ${BACKOFF_DELAY_MS / 1000}s...`,
      );
...
async function fetchSquadData(clubs: Club[]) {
  ...
  if (cachedSquads && Object.keys(cachedSquads).length >= expectedMinPlayers) {
    console.log(
      `  Using cached squad data (${Object.keys(cachedSquads).length} players)`,
    );
    ...
    for (const [id, sp] of Array.from(playerMap.entries())) {
      if (
        sp.fromClubs.length === 1 &&
        sp.fromClubs[0] === "manual" &&
    ...
    if (injected > 0 || removed > 0) {
      ...
      console.log(
        `  +${injected} added, -${removed} removed → total: ${playerMap.size}`,
      );
    }
  ...
  for (const club of clubs) {
    ...
    for (const season of DISCOVERY_SEASONS) {
      const result = await apiFetch<any>(
        `/clubs/${club.id}/players?season_id=${season}`,
      );
      ...
      for (const p of result.players) {
        ...
        if (existing) {
          ...
          if (season < existing.earliestSeason)
            existing.earliestSeason = season;
          const mv = parseMarketValue(p.marketValue);
          if (mv && (!existing.marketValue || mv > existing.marketValue))
...
async function fetchPlayerData(
  ...
  const fullyDone = toFetch.filter(
    (d) =>
      cache[d.id] && ENDPOINTS.every((ep) => !needsFetch(cache[d.id]?.[ep])),
  ).length;
  ...
  console.log(
    `    Partial  (some nulls):           ${partialN}  → will fetch missing only`,
  );
  console.log(
    `    Empty    (nothing cached yet):   ${emptyN}  → will fetch all`,
  );
    ...
    for (const ep of missing) {
      ...
      if (data === null) {
        process.stdout.write(
          ` ⚠ Blocked. Waiting ${BACKOFF_DELAY_MS / 1000}s...`,
        );
      } else {
        cache[id][ep] = data;
        anyFetched = true;
...
function parseAndWrite(
  ...
  const existingMap = new Map(
    enrichedFootballPlayers.map((p) => [p.playerId, p]),
  );
  const diffs: any[] = [];
  for (const np of processed) {
    ...
    const removed = (old.achievements ?? []).filter(
      (a: string) => !newAch.has(a),
    );
  ...
  const tiers = {
    ...
    wellKnown: processed.filter((p) => p.fameScore >= 40 && p.fameScore < 70)
      .length,
    known: processed.filter((p) => p.fameScore >= 20 && p.fameScore < 40)
      .length,
